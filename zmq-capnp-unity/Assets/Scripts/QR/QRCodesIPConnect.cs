...
{
    public class QRCodesIPConnect : MonoBehaviour
    {
        ...
        private void InstantiateQRCode(ActionData action)
        {
            ...
            // if the data is an ipv4 address with three dots, then it is an ip address
            if (qrCodeData.Contains(".") && qrCodeData.Split('.').Length == 4)
            {
                connectIP.Connect(qrCodeData);
                Debug.Log("Connecting to qr ip" + qrCodeData);
            }
        }
